...
class Decorate extends _Events.default {
  ...
  decorateStorageEntry(creator, decorateMethod) {
    ...
    const decorated = this.hasSubscriptions && creator.iterKey ? creator.meta.type.asMap.kind.isLinkedMap ? this.decorateStorageLinked(creator, decorateMethod) : this.decorateStoragePrefixed(creator, decorateMethod) : decorateMethod(function () {
  ...
  decorateStorageLinked(creator, decorateMethod) {
    var _this2 = this;
    ...
    return decorateMethod(function () {
      ...
      return args.length ? _this2._rpcCore.state.subscribeStorage([[creator, ...args]]).pipe((0, _operators.map)((_ref10) => {
        let [data] = _ref10;
        return data;
      })) : _this2._rpcCore.state.subscribeStorage([creator.iterKey]).pipe((0, _operators.switchMap)((_ref11) => {
        let [key] = _ref11;
  ...
  decorateStoragePrefixed(creator, decorateMethod) {
    var _this3 = this;
    ...
    return decorateMethod(function () {
      ...
      return args.length ? _this3._rpcCore.state.subscribeStorage([[creator, ...args]]).pipe((0, _operators.map)((_ref12) => {
        let [data] = _ref12;
        return data;
      })) : _this3._rpcCore.state // FIXME This should really be some sort of subscription, so we can get stuff as
      // it changes (as of now it is a one-shot query). Not sure how to do this though...
      .getKeys(creator.iterKey).pipe((0, _operators.switchMap)(keys => _this3._rpcCore.state.subscribeStorage(keys.map(key => key.setOutputType(outputType)))));
    });
  }
