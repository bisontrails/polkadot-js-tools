...
function createErrorMessage(_ref, error) {
  let {
    method,
    params,
    type
  ...
  return "".concat(method, "(").concat(inputs, "): ").concat(type, ":: ").concat(error.message);
}
/**
 * @name Rpc
...
class Rpc {
  ...
  constructor(registry, provider) {
    let userRpc = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  ...
  createMethodSend(method) {
    ...
    const call = function call() {
      ...
      return (0, _rxjs.of)(1).pipe((0, _operators.map)(() => _this.formatInputs(method, values)), (0, _operators.switchMap)(params => (0, _rxjs.combineLatest)([(0, _rxjs.of)(params), (0, _rxjs.from)(_this.provider.send(rpcName, params.map(param => param.toJSON())))])), (0, _operators.map)((_ref5) => {
        ...
        const message = createErrorMessage(method, error); // don't scare with old nodes, this is handled transparently
    ...
    const call = function call() {
      ...
      return new _rxjs.Observable(observer => {
        ...
        return () => {
          ...
          subscriptionPromise.then(subscriptionId => (0, _util.isNumber)(subscriptionId) ? _this2.provider.unsubscribe(subType, unsubName, subscriptionId) : Promise.resolve(false)).catch(error => l.error(createErrorMessage(method, error)));
        };
      }).pipe((0, _rxjs2.drr)());
    };
  ...
  formatInputs(method, inputs) {
    ...
    return inputs.map((input, index) => (0, _codec.createTypeUnsafe)(this.registry, method.params[index].type, [input]));
  }
  ...
  formatOutput(method, params, result) {
    ...
    return (0, _codec.createTypeUnsafe)(this.registry, method.type, [result]);
  }
  ...
  formatStorageData(key, value) {
    ...
    if (meta.modifier.isOptional) {
      return new _types.Option(this.registry, (0, _types.createClass)(this.registry, type), isEmpty ? null : (0, _codec.createTypeUnsafe)(this.registry, type, [input], true));
    }

    return (0, _codec.createTypeUnsafe)(this.registry, type, [isEmpty ? meta.fallback : input], true);
  }
